...
def test_np_possession_scope_dossier_is_candidate_scoping_only():
    ...
    assert "review notes now exist" in text
    assert "dictionary slice for np structure / possession does not yet exist" in text
...
def test_np_possession_scope_dossier_recommends_basic_np_order_next():
    ...
    assert "review_notes_np_possession.md" in text
