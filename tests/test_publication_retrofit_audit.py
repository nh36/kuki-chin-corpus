...
def test_publication_retrofit_audit_recommends_priority():
    ...
    assert "first analyzer-aware candidate layer, curated extractor route, first candidate-controlled dossier, and first grammar slice now exist; dictionary and review-note slices have not yet begun" in text
    assert "Advance sentence-final particles conservatively from grammar print slice to dictionary print slice" in text
...
def test_progress_marks_current_packets_as_review_ready_and_not_case_marking_next():
    ...
    assert "3. [ ] Keep broad TAM, directionals, chrestomathy, Mizo/lus, and the other Kuki-Chin languages deferred while the sentence-final particles grammar slice remains the active narrow next step." in text
...
def test_remaining_retrofit_inventory_exists_and_distinguishes_current_vs_future_topics():
    ...
    assert "tests/test_sentence_final_particles_dossier.py" in text
    assert "tests/test_sentence_final_particles_print_slice.py" in text
    assert "active narrow grammar-slice stage only; dictionary and review-note slices have not yet begun" in text
    assert "sentence-final particles grammar print slice now exists while dictionary and review-note slices have not yet begun" in text
