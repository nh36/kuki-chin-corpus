...
def test_publication_retrofit_audit_recommends_priority():
    ...
    assert "tests/test_directionals_review_notes.py" in text
    assert "first analyzer-aware candidate layer, first candidate-controlled dossier, first grammar slice, first dictionary slice, and review notes now exist; the packet is ready for human review at the current slice maturity level" in text
    assert "hold stable for maintenance and human review; if a directionals defect is identified later" in text
...
def test_progress_marks_current_packets_as_review_ready_and_not_case_marking_next():
    ...
    assert "review_notes_directionals.md" in text
    assert "The directionals packet is now ready for human review at the current slice maturity level." in text
    assert "1. [ ] Keep demonstratives/deixis, negation, pronouns/clusivity, stem alternation, case marking, interrogatives, numerals, quantifiers, coordinators, sentence-final particles, and directionals stable for maintenance and human review." in text
    assert "2. [ ] Do not begin a new narrow publication-review grammar packet automatically; route any later packet work through explicit scope selection after human review of the completed packets." in text
...
def test_remaining_retrofit_inventory_exists_and_distinguishes_current_vs_future_topics():
    ...
    assert "review_notes_directionals.md" in text
    assert "tests/test_directionals_review_notes.py" in text
    assert "directionals packet is also ready for human review at the current slice maturity level" in text
    assert "Deferred future or non-slice topics" in text
    for deferred in ("broad TAM / aspect / modal", "chrestomathy", "Mizo/lus"):
