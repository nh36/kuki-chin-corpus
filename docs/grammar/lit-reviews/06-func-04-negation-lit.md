# Literature Review: Negation in Tedim Chin

This report compiles previous scholarship on negation in Tedim Chin.

---

## 1. Henderson (1965)

Henderson identifies **-kei** as a negative adverb but provides limited systematic treatment:

> "'ke? is a fusion of the adverb -kei and the sentence particle le?" (p. 20)

Key forms in her texts:
- `kuan nuam kei a` 'he didn't want to go to work'
- `kuan nawn kei` 'did not go any more'
- `sam kik zo kei hi` 'nobody could call them back'
- `nui nui kei un` 'do not laugh all the time' (prohibitive)

The negative **-kei** is described as an adverb that attaches to verbs.

---

## 2. Zam Ngaih Cing (2017/2018)

### 2.1 Negative Markers (§5.8.2.1)

> "There are two types of negative markers in Tedim Chin: **-keí [≈-kei]** and **-lɔù [≈-lo]**. These negative morphemes are marked on the verb."

**Distribution by verb stem:**

| Marker | Stem | Alternant |
|--------|------|-----------|
| -lɔù [≈-lo] | Stem 1 | -lɔùʔ [≈-loh] |
| -lɔùʔ [≈-loh] | Stem 2 | — |
| -keí [≈-kei] | Both stems | — |

> "These two can never interchange their position."

### 2.2 -lɔù / -lɔùʔ NEG

Used with irrealis **díŋ [≈ding]**:
```
náŋ   íʔmú-lɔù      díŋ
2SG   sleep.1-NEG.1 PURP
'You will not sleep'
```

### 2.3 -keí NEG

Used with imperatives and first person:
```
íʔmú-keí      ɔú
sleep.1-NEG   IMP
'You don't sleep' (prohibitive)

keí   paī-kéŋ
1SG   go.1-NEG.1SG.RLS
'I didn't go'
```

> "The first person agreement markers occur with the negative marker -keí [≈-kei]"

**Ungrammatical**: *keí [≈kei] paī-lɔú [≈pai-lo] ìŋ [≈ing] (cannot use -lɔù [≈-lo] with 1SG realis)

### 2.4 Informal Negative -dàʔ (§5.8.2.1)

> "Another negative morpheme **-dàʔ [≈-dah]** is not commonly used in written form but it is usually used in informal speech."

```
né-dàʔ      níŋ
eat.1-NEG   1SG.IRR
'I will rather not eat'
```

Note: **dàʔ [≈dah]** also means 'sad' as a stative verb:
```
dàʔ   lua     iŋ
sad   very    1SG.RLS
'I am very sad'
```

### 2.5 Negative with Verb Stems (§5.5.4)

> "Stem 1 and Stem 2 verbs can occur with negative particle. It differs in meaning."

**Stem 1** (ongoing/stative reading):
```
naūŋék    kàp-lɔù
baby      cry.1-NEG.1
'The baby is not crying'
```

**Stem 2** (perfective/resultative reading):
```
naūŋék    kàɁ-lɔù
baby      cry.2-NEG.1
'Didn't cry for the baby' (i.e., no crying happened)
```

### 2.6 Verb-Negation Order (§7.3.13)

> "The order of negative marker and verb is VNeg. Dryer (2008: 72) stated that 'negative morphemes in Tibeto-Burman languages are either affixes or particles'. This holds true for Tedim Chin."

```
túnī      guàɁ  zù-lɔù
today     rain  pour.1-NEG.1
'Today, it didn't rain'
```

### 2.7 Negative Imperative/Prohibitive (§7.4.2.3)

> "In negative imperative or prohibitive, the negative particle keí [≈kei] is used to negate the imperative clause."

```
paī   keí   ɔú
go.1  NEG   IMP
'Don't go'

taī   keí   ɔú
run.1 NEG   IMP
'Don't run'
```

---

## 3. Singh (2018): Sukte Grammar [Comparative]

### 3.1 Negators (§8.7)

> "Negation in Sukte is formed at the clausal level but not at the constituent level... The negators used in the language are **lo** and **kei**."

### 3.2 kei with Verbal Predicates (§8.7.1.1)

> "In Sukte, the negator kei are used to negate the verbal predicate."

```
buʔsiʔ    kə      ne     kei   hi
rice      1SG     eat    NEG   DECL
'I do not eat rice.'

əməʔ      boŋnoi  ə      don   kei   hi
he        milk    3SG    drink NEG   DECL
'He does not drink milk.'
```

Also with possessives:
```
laikuŋ    kə      nei    kei   hi
pen       1SG     POSS   NEG   DECL
'I do not have pen.'
```

### 3.3 lo with Nominal Predicates (§8.7.1.2)

> "In Sukte, the negator lo is used with prefix hi for emphasising to negate nominal predicate."

```
əməʔ  siəvuən   hi    lo    hi
he    doctor    EMP   NEG   COP
'He is not a doctor.'

pəlik    kə     hi    lo    hi
police   1SG    EMP   NEG   COP
'I am not a police.'
```

### 3.4 Negative Imperative (§8.1.2.4)

> "Prohibitive sentences are formed by postposing the morpheme ken to the verb root. It is worth mentioning here that the morpheme **ken** is mutually derived from the **kei** and **in**."

```
mi       tai    ken
person   chide  NEG.IMP
'Do not chide to others.'

sə       thət   ken
animal   kill   NEG.IMP
'Do not kill animal.'
```

---

## 4. Synthesis

### 4.1 Negator Inventory

| | Tedim | Sukte | Function |
|---|-------|-------|----------|
| General verbal | -keí [≈-kei] | kei | Main clausal negator |
| Stem-sensitive | -lɔù [≈-lo]/-lɔùʔ [≈-loh] | — | Stems 1/2 |
| Informal | -dàʔ [≈-dah] | — | Colloquial |
| Nominal | — | lo | Copular/nominal |
| Prohibitive | keí [≈kei] + ɔú [≈o] | ken | Negative imperative |

### 4.2 Structural Properties

**Common features**:
- Post-verbal position (V-NEG order)
- Clausal scope (not constituent negation)

**Tedim-specific**:
- Stem alternation affects negator form (-lɔù [≈-lo] vs. -lɔùʔ [≈-loh])
- Semantic difference between Stem 1-NEG (ongoing) vs. Stem 2-NEG (perfective)

**Sukte-specific**:
- Distinct negators for verbal (kei) vs. nominal (lo) predicates
- Fused prohibitive form (ken < kei + in)

### 4.3 Distinctive Findings

**Zam Ngaih Cing's unique contributions**:
1. Two-negator system (-keí [≈-kei] vs. -lɔù [≈-lo])
2. Stem-sensitive negator alternation
3. Informal register negator -dàʔ [≈-dah]
4. Semantic difference tied to verb stem selection

**Singh's unique contributions**:
1. Verbal vs. nominal predicate negation split
2. Etymology of prohibitive ken < kei + in

---

## 5. Open Questions

1. **Etymology**: Are Tedim -keí [≈-kei] and Sukte kei cognate? What is the source?
2. **-lɔù [≈-lo] origin**: Is -lɔù [≈-lo] related to Mizo lo or a separate development?
3. **Scope**: What is the interaction of negation with focus, quantifiers?
4. **Double negation**: Are there constructions with multiple negators?
5. **-dàʔ [≈-dah] etymology**: Is the 'sad' meaning primary or derived?

---

## References

Henderson, E. J. A. (1965). *Tiddim Chin*. London: Oxford University Press.

Singh, L. S. (2018). *A Descriptive Grammar of Sukte*. Imphal: Grassroot Publications.

Zam Ngaih Cing. (2017/2018). *A Grammar of Tedim Chin*. Payap University.
