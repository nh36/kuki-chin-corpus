# Directional Affixes - Literature Review

This report surveys previous scholarship on directional affixes (andative *va-*, venitive *ong-/hong-*) in Tedim Chin, organized chronologically. Special attention is given to distinctions drawn by one author but not others.

---

## Overview

Directional affixes are a characteristic feature of Tibeto-Burman languages, marking motion toward or away from the deictic center (typically the speaker). In Tedim Chin, two primary directional markers have been identified:

| Scholar | Year | "Away" (Andative) | "Toward" (Venitive) | Position |
|---------|------|-------------------|---------------------|----------|
| Henderson | 1965 | *va-* | *hong* | Pre-verbal |
| Zam Ngaih Cing | 2018 | *và* | *ɔ́ŋ/hɔ́ŋ* | Pre-verbal | <!-- ≈Bible: va awng hawng -->
| Otsuka | 2017 | *va³-* | *oŋ¹-/hoŋ¹-* | Pre-verbal prefix | <!-- ≈Bible: ong hong -->

---

## Henderson 1965

**Source:** Henderson, Eugénie J.A. 1965. *Tiddim Chin: A Descriptive Analysis of Two Texts*. London: Oxford University Press.

Henderson's treatment of directional affixes is embedded within her analysis of verbal figures and colloquial style rather than presented as a separate category. She uses the notation "vpf" (verbal prefix) in her structural formulae.

### The Verbal Prefix *va-*

Henderson documents *va-* in several contexts but does not explicitly label it a "directional." Her examples show it marking motion away:

- **a vakuan** 'he went off to work' (I 8) — analyzed as ppf.-vpf.-v.¹
- **va-sam hen** 'let him go and call' — 'amah pai² hen la, a upa **va-sam¹** hen' ("let him go and call his elder brother")

### The Verbal Prefix *na-*

Henderson documents a distinct prefix *na-* marking action in the absence of the speaker or hearer:

> The verbal prefix *na-* appears with the meaning "in his absence" or "unbeknown to him."

Examples from Henderson:
- **na ne ne** 'eating away unbeknown to him' (I 39)
- **na po kik** 'growing again in his absence' (I 10)
- **anasuak pah pah** 'it immediately became in his absence' (I 12)

### The Marker *hong*

Henderson discusses *hong* in connection with the colloquial style and subjunctive mood:

> "Such sentences as have been recorded frequently contain the figure 'hi ci' *like this*, or the word **'hong'** (see p. 113). Many subjunctive sentences recorded convey the sense of doing something for someone else." (p. 84)

Henderson contrasts indicative and subjunctive sentences:

| Indicative | Subjunctive |
|------------|-------------|
| *kei a ding in lup¹ hi* 'he prepared beer on my behalf' | *zu hong lup¹ᴵ hi* 'he prepared beer for me' |

She further notes:

> "'Zu hong lup¹ᴵ hi' *he prepared beer for me* may be contrasted with 'hong pai¹ hi' *he came to see me* (not on my behalf)." (p. 85)

### Diachronic Note

Henderson provides a crucial note on the etymology of *hong*:

> "Henderson (1965) によれば，hoŋ がティディム近郊のヴァルヴムでは動詞「来る」として用いられる (Henderson 1965: 113)."
[≈Bible: hong]

(According to Henderson 1965, *hoŋ* is used as the verb 'come' in Valvum, near Tedim.)
[≈Bible: hong]

### Key Distinctions in Henderson

1. **Absence prefix *na-***: Henderson identifies a distinct prefix *na-* for actions performed in someone's absence—a category not explicitly discussed by later scholars as a separate directional
2. **Mood connection**: Henderson links *hong* to subjunctive mood and benefactive meaning, not just directional motion
3. **Register distinction**: She notes that *hong* appears particularly in the colloquial style

---

## Zam Ngaih Cing 2018

**Source:** Zam Ngaih Cing. 2018. *A Grammar of Tedim Chin*. PhD Dissertation, North Eastern Hill University, Shillong. Chapter 5, §5.8.1.3.

Zam Ngaih Cing provides a more systematic treatment of directional affixes as a distinct morphological category within the verb phrase structure.

### Pre-Verbal Directionals

> "The pre-verb slot can be occupied by the directional markers **ɔ́ŋ/hɔ́ŋ** and **và** which both refers to 'away' and 'towards' from the reference point or deictic center. **và** is used when the goal is away from the subject. **ɔ́ŋ/hɔ́ŋ** is used when the goal is directed towards the speaker." (§5.8.1.3)
[≈Bible: awng hawng va va awng hawng]

### Examples

**Andative *và* (motion away):**
[≈Bible: va]
> *và paī ún* 'Go!' (lit. 'away go IMP.PL')
[≈Bible: va pai un]

**Venitive *ɔ́ŋ* (motion toward):**
[≈Bible: awng]
> *klmāŋ-pá-n à zì díŋ á ā **ɔ́ŋ** lá-xá díŋ hì*
[≈Bible: klmang-pa-n zi ding awng la-kha ding hi]
> 'The Burmese officer might take you unexpectedly as/for his wife'

### Co-occurrence of Directionals

Zam Ngaih Cing makes an important observation about ordering:

> "When both the directional markers *và* and *ɔ́ŋ* occur adjacent to each other, it is always *ɔ́ŋ* which precedes the other marker but not the other way." (§5.8.1.3.1)
[≈Bible: va awng awng]

Example:
> *vān-ā āksínélkaī **ɔ́ŋ và** lák-sàk núŋ cì*
[≈Bible: van-a aksinelkai awng va lak-sak nung ci]
> 'They said, "We will go and get comet from the sky"'

### Other Directionals

Zam Ngaih Cing identifies an additional directional marker:

> "In Tedim Chin, a directional marker denoting 'forward or advance' is identified with **mā-**. It means front or ahead." (§5.8.1.3.2)
[≈Bible: ma]

Examples:
- *mánɔ̀Ɂ* 'approach front'
[≈Bible: manawh]
- *málép* 'action done before the actual time'
[≈Bible: malep]
- *mápaī* 'going smoothly' vs. *má paì* 'go in advance'
[≈Bible: mapai ma pai]
- *mákaì* 'leader' vs. *mā kaì* 'lead the front'
[≈Bible: makai ma kai]

### Post-Verbal Directional Marking

Zam Ngaih Cing distinguishes pre-verbal directionals (deictic) from post-verbal directional suffixes (elevational):

> "Verbal suffixes are used to denote three types of directions—upward, downward and horizontal (level). The upward and downward direction are opposite of each other. Upward direction is similar with north and downward is comparable with south." (§5.8.2.3)

| Direction | Suffix | Example |
|-----------|--------|---------|
| Upward | *-tɔù* | *kàʔ-tɔù* 'ascend upward' | <!-- ≈Bible: -to kah-to -->
| Downward | *-sùk* | *kúm-sùk* 'descend downward' | <!-- ≈Bible: -suk kum-suk -->
| Horizontal | *-pʰeī* | *paī-pʰeī* 'go level (straight)' | <!-- ≈Bible: -phei pai-phei -->

Locational directionals:
- *sàk-lám-àʔ* 'towards the higher side'
[≈Bible: sak-lam-ah]
- *xāŋ-lám-àʔ* 'towards the lower side'
[≈Bible: khang-lam-ah]

### Key Distinctions in Zam Ngaih Cing

1. **Systematic classification**: She treats directionals as a distinct grammatical category within verb phrase structure
2. **Co-occurrence constraints**: She documents the fixed order ɔ́ŋ-và when both occur
[≈Bible: awng-va]
3. **Elevational vs. deictic**: She clearly distinguishes pre-verbal deictic directionals from post-verbal elevational suffixes
4. **Additional marker *mā-***: She identifies 'forward/advance' as a separate directional category
[≈Bible: ma]

---

## Otsuka 2017

**Source:** Otsuka, Yukinori (大塚行誠). 2017. "ティディム・チン語とジンポー語における方向接辞の対照" [A Contrastive Study of Directional Affixes in Tiddim Chin and Jingpho]. Conference paper.

Otsuka provides the most detailed theoretical treatment of Tedim Chin directional affixes, using comparative data from Jingpho and explicit reference to typological literature on grammaticalization.

### Terminology

Otsuka adopts cross-linguistic terminology:
- **Andative** (*去辞*): *va³-* — motion away from deictic center
- **Venitive** (*来辞*): *oŋ¹-/hoŋ¹-* — motion toward deictic center
[≈Bible: ong hong]

### Morphological Position

> "ティディム・チン語の去辞 va³-と来辞 oŋ¹-は，どちらも動詞の前に現れる接頭辞である"
[≈Bible: ong]
> (Tedim Chin's andative *va³-* and venitive *oŋ¹-* both appear as prefixes before the verb.)
[≈Bible: ong]

Verb complex structure:
> (Person)=(Directional)-(Reflexive/Reciprocal)-VERB=(Auxiliary)=(TAM/Number/Person)

### The Andative *va³-*

**Basic meaning:**
> "ティディム・チン語の去辞 va³-およびジンポー語の去辞-s は，移動物が談話参加者の視点や領域から離れる物理的移動を表す"
> (The andative marks physical movement where the moving entity departs from the perspective or domain of discourse participants.)

Example:
> *tut³naː² **va³-**la¹=pak³=niŋ¹ =ei³*
[≈Bible: ning]
> 'I'll just go get a seat over there'

**Semantic restrictions:**
Otsuka notes that unlike related languages (Mizo, Lai), Tedim Chin's andative does *not* have derived meanings of excess or exclamation:

> "ティディム・チン語の去辞 va³-にそのような派生の用法は見られない。va³-は，専ら物理的移動に関する事を示すと考えられる。"
> (Tedim Chin's andative *va³-* does not exhibit such derived usages. It appears to be restricted to physical movement.)

### The Venitive *oŋ¹-/hoŋ¹-*

**Basic meaning:**
> "ティディム・チン語の来辞 oŋ¹-およびジンポー語の来辞-r は，移動物が談話参加者の視点や領域に向かう物理的移動を表す"
[≈Bible: ong]
> (The venitive marks physical movement where the moving entity approaches the perspective or domain of discourse participants.)

Example:
> *siŋ¹hiaŋ² **oŋ¹-**kia³=in²teʔ¹*
[≈Bible: sing hiang ong teh]
> 'A branch will fall (toward me/you/us)'

**Derived meanings:**

Unlike the andative, the venitive has extensive derived uses:

1. **Change of state:**
> *tua² haːi² =peːn² dam²dam²=in² **oŋ¹-**naːi²=taː³*
[≈Bible: ong]
> 'That mango has gradually become yellow' (VEN-yellow)

2. **Emergence/Appearance:**
> *oŋ¹-suak¹* (VEN-born) 'to be born'
[≈Bible: ong]
> *oŋ¹-doːk¹* (VEN-appear) 'to appear'
[≈Bible: ong]

3. **Obligatory with speech act participant affected:**

When the patient, recipient, or causee includes a speech act participant (1st or 2nd person), *oŋ¹-* is obligatory:
[≈Bible: ong]

> *kei¹ **oŋ¹-**tʰei³=ni¹teʔ³* 'You know me'
[≈Bible: ong thei teh]
> \**kei¹ tʰei³=ni¹teʔ³* (ungrammatical without *oŋ¹-*)
[≈Bible: thei teh ong]

> *naŋ¹ let¹suŋ² xat³ **oŋ¹-**pia¹=iŋ³* 'I gave you a present'
[≈Bible: nang sung ong ing]

4. **With valency-increasing suffixes:**

When *-sak* (substitutive/benefactive), *-piʔ* (associative), or *-san* (relinquitive) add a speech act participant, *oŋ¹-* is required:
[≈Bible: -pih ong]

> *lian³ =in³ kei¹ **oŋ¹-**vaːk³-piʔ³* 'Lian went out with me' (VEN-walk-ASS)
[≈Bible: ong -pih]

### Co-occurrence of Andative and Venitive

Otsuka documents that *oŋ¹-* and *va³-* can co-occur:
[≈Bible: ong]

> *tua² ui¹ **oŋ¹-va³-**hoːl³kʰiat¹-sak³=iŋ³ =ei³*
[≈Bible: ong khiat ing]
> 'I drove that dog away for your sake' (VEN-AND-drive.away-SUBS)

In such cases, *oŋ¹-* no longer marks physical motion toward the speaker but rather functions as an **inverse marker** indicating that a speech act participant is affected.
[≈Bible: ong]

### Interaction with *na-* Prefix

Otsuka cites Henderson's *na-* prefix and shows it can co-occur with the venitive:

> *nu¹-ciŋ² =in³ naŋ¹ **oŋ¹-na¹-**taːi³-san³ =moː³*
[≈Bible: -cing nang ong]
> 'Aunt Cing left you behind and ran off' (VEN-NA-run-RELNQ)

### Register Variation

> "ティディム・チン語の来辞 oŋ¹-は，フォーマルな場面では，hoŋ¹-という形式でも用いる。"
[≈Bible: ong hong]
> (In formal contexts, the venitive appears as *hoŋ¹-*.)
[≈Bible: hong]

### Historical Etymology

Otsuka traces the origins:

**Venitive *oŋ-/hoŋ-*:**
[≈Bible: ong hong]
- Related to PKC \**huŋ* 'COME' (VanBik 2009)
[≈Bible: hung]
- *hoŋ* used as the verb 'come' in Valvum dialect (Henderson 1965: 113)
[≈Bible: hong]

**Andative *va-*:**
- Cognates found throughout Kuki-Chin (DeLancey 1985: 372-374)
- Related to PTB \**s-wa* 'GO' (VanBik 2017)

### Key Distinctions in Otsuka

1. **Asymmetry between andative and venitive**: The venitive is far more developed, with obligatory uses and derived meanings; the andative is restricted to physical motion
2. **Inverse function**: When *oŋ¹-* co-occurs with *va³-*, it functions as an inverse marker rather than a directional
[≈Bible: ong]
3. **Speech act participant requirement**: The venitive becomes obligatory when any added argument (patient, beneficiary, etc.) is a speech act participant
4. **Grammaticalization pathway**: Otsuka situates the venitive's development within Heine & Kuteva's (2002) COME > VENITIVE pathway

---

## Comparative Summary

### Points of Agreement

All three scholars agree on:
- Two primary pre-verbal directionals exist: andative (*va-*) and venitive (*ong-/hong-*)
- Both are prefixes that occur before the verb
- The venitive has formal register variant *hong-*
- Directionals derive historically from motion verbs

### Key Differences and Distinctions

| Feature | Henderson 1965 | Zam Ngaih Cing 2018 | Otsuka 2017 |
|---------|----------------|---------------------|-------------|
| **Absence prefix *na-*** | ✓ Documented | Not discussed | References Henderson |
| **Elevational suffixes** | Not discussed | ✓ *-tou, -suk, -phei* | Not discussed |
| **Forward marker *ma-*** | Not discussed | ✓ Documented | Not discussed |
| **Obligatory venitive** | Implied (mood) | Not discussed | ✓ Detailed rules |
| **Inverse function** | Not discussed | Not discussed | ✓ With *va-* co-occurrence |
| **Mood connection** | ✓ Subjunctive | Not emphasized | Not emphasized |
| **Typological framing** | Prosodic analysis | Descriptive | ✓ Grammaticalization |

### Unique Contributions

**Henderson (1965):**
- Documents the *na-* prefix for absent participant
- Links *hong* to subjunctive mood and benefactive meaning
- Notes dialect evidence (*hong* = 'come' in Valvum)

**Zam Ngaih Cing (2018):**
- Systematic treatment within verb phrase structure
- Documents elevational suffixes as separate category
- Identifies forward/advance marker *mā-*
[≈Bible: ma]
- Notes fixed order constraint: *ɔ́ŋ-và* (never \**và-ɔ́ŋ*)
[≈Bible: awng-va va-awng]

**Otsuka (2017):**
- Most theoretically sophisticated treatment
- Documents obligatoriness of venitive with SAP-affected arguments
- Identifies inverse function when venitive co-occurs with andative
- Notes asymmetry: venitive highly developed, andative restricted
- Provides Proto-Tibeto-Burman etymologies

---

## Gaps in the Literature

1. **Corpus-based frequency**: No study provides systematic frequency data on directional usage
2. **Processing/acquisition**: No psycholinguistic or acquisition studies
3. **Discourse functions**: Limited analysis of how directionals function in narrative structure
4. **Dialectal variation**: Only Henderson notes the Valvum data; more comparative data needed
5. **Interaction with other prefixes**: The full paradigm of prefix combinations needs documentation
6. **Semantic bleaching**: The extent to which *oŋ-* has grammaticalized beyond motion is not fully quantified
[≈Bible: ong]

---

## References

- Button, Christopher. 2011. *Proto Northern Chin*. STEDT Monograph Series.
- Chhangte, Lalnunthangi. 1986. *A preliminary grammar of the Mizo language*. MA thesis, University of Texas at Arlington.
- DeLancey, Scott. 1980. *Deictic categories in the Tibeto-Burman verb*. PhD dissertation, Indiana University.
- DeLancey, Scott. 1985. The analysis-synthesis-lexis cycle in Tibeto-Burman. In John Haiman (ed.), *Iconicity in Syntax*, 367-389.
- Heine, Bernd & Tania Kuteva. 2002. *World Lexicon of Grammaticalization*. Cambridge University Press.
- Henderson, Eugénie J.A. 1965. *Tiddim Chin: A Descriptive Analysis of Two Texts*. Oxford University Press.
- Otsuka, Yukinori (大塚行誠). 2009. ティディム・チン語における方向接頭辞 óŋ-. *東京大学言語学論集* 28: 197-218.
- Otsuka, Yukinori. 2017. ティディム・チン語とジンポー語における方向接辞の対照. Conference paper.
- So-Hartmann, Helga. 2009. *A Descriptive Grammar of Daai Chin*. STEDT.
- VanBik, Kenneth. 2009. *Proto-Kuki-Chin*. STEDT Monograph 7.
- VanBik, Kenneth. 2017. Directional Pre-verbal Particles in Hakha Lai. *Himalayan Linguistics* 16(1).
- Zam Ngaih Cing. 2018. *A Grammar of Tedim Chin*. PhD Dissertation, NEHU Shillong.
