...
| Form or pattern | Rough function | Example context | Current grammar-facing status | Boundary issue |
| --- | --- | --- | --- | --- |
| `kanei` | agreement before a verbal host | `ka nei` clauses | well supported | broader person-prefix paradigm remains open |
| `ka-nei` | segmented agreement form | `1SG-have` parsing | well supported | does not settle all verbal prefix routing |
| `kainn` | possession before a nominal host | `ka inn` clauses | well supported | broader possessor syntax remains open |
| `ka-inn` | segmented possessive form | `1SG.POSS-house` parsing | well supported | does not settle apostrophe possession |
| `ainn` | third-person nominal prefix row | `a inn` phrases | boundary material | shared `a-` family overlaps other domains |
| `ipai` | independent pronoun/clusivity boundary row | `i pai` patterns | boundary material | belongs mainly with pronoun and clusivity analysis |
| `hongmu` / `kongmu` | object-prefix or inverse-like boundary row | `hong mu` / `kong mu` patterns | boundary material | selection conditions remain unsettled |
| `kipan` | `ki-` boundary row | `hong kipan` clauses | boundary material | overlaps reflexive/middle and derivational analysis |
| apostrophe possession | NP possessor marking boundary | `Topa' inn` | boundary material | belongs mainly with broader possession analysis |
| `a bawl mi` / `ahih` | relative-clause boundary rows | relative or clause-linkage contexts | boundary material | overlaps relative-clause and clause-linkage structure |
...
The form `kipan` 'from / begin / REFLEXIVE-boundary' remains boundary-only material. It overlaps reflexive/middle analysis and wider derivational behavior, so it is not treated here as a settled agreement prefix.
