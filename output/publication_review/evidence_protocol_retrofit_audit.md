...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| numerals | yes (`dossier_numerals.md`) | no | no | no | yes (`candidates_numerals.tsv`) | `tests/test_numerals_candidates.py`, `tests/test_numerals_dossier.py`, `tests/test_publication_candidate_extractor.py` | no report-level correction in this commit; the existing generated report remains the discovery source | `kua` = nine vs `kua` = who, `khat` = one vs indefinite, numeral/classifier boundary, report-style distributive reduplication, and raw-count spillover from generated prose | first analyzer-aware candidate layer, curated extractor route, and candidate-controlled dossier now exist; grammar, dictionary, and review-note slices have not started yet | keep the route narrow, use the candidate TSV plus dossier as the control, and draft the grammar slice only after the dossier wording has settled |
...
3. **Continue numerals through the candidate-first sequence** — the first analyzer-aware `candidates_numerals.tsv` layer and `dossier_numerals.md` now exist, but grammar, dictionary, and review-note slices have not started yet.
