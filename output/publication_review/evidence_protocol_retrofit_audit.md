...
With negation now retrofitted and pronouns / clusivity now hardened, the remaining practical order should be:

1. **Stem alternation next** — it now becomes the clearest remaining packet that still lacks explicit candidate rows.
...
The retrofit audit now supports a clear updated next step: move to the stem alternation retrofit rather than opening a brand-new grammar topic. Demonstratives remains the protocol pilot, negation is the first hardened retrofit, pronouns / clusivity is now the second hardened retrofit, and the remaining backlog should move toward the same candidate-first architecture in the order above.
