...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| numerals | yes (`dossier_numerals.md`) | yes (`grammar_numerals_print_slice.md`) | no | no | yes (`candidates_numerals.tsv`) | `tests/test_numerals_candidates.py`, `tests/test_numerals_dossier.py`, `tests/test_numerals_print_slice.py`, `tests/test_publication_candidate_extractor.py` | no report-level correction in this commit; the existing generated report remains the discovery source | `kua` = nine vs `kua` = who, `khat` = one vs indefinite, numeral/classifier boundary, report-style distributive reduplication, and raw-count spillover from generated prose | first analyzer-aware candidate layer, curated extractor route, candidate-controlled dossier, and first grammar slice now exist; dictionary and review-note slices have not started yet | keep the route narrow, use the candidate TSV plus dossier plus grammar slice as the control, and draft the dictionary slice only after the grammar wording has settled |
...
3. **Continue numerals through the candidate-first sequence** — `candidates_numerals.tsv`, `dossier_numerals.md`, and `grammar_numerals_print_slice.md` now exist, but dictionary and review-note slices have not started yet.
4. **Keep quantifiers, coordinators, and sentence-final particles deferred until numerals is reviewed**.
5. **Keep broad TAM, directionals, chrestomathy, Mizo/lus, and the other Kuki-Chin languages deferred unless they are explicitly chosen as a new scope**.

## Conclusion

The retrofit audit now supports a clearer roadmap. Demonstratives remains the protocol pilot, negation is the first hardened retrofit, pronouns / clusivity is the second hardened retrofit with its `ko` analyzer issue addressed upstream, stem alternation is the third completed retrospective retrofit at the publication-review evidence-architecture level, case marking is now fully aligned at the same candidate-first slice maturity level, interrogatives now has a candidate-controlled dossier plus first grammar, dictionary, and review-note surfaces in place, and numerals now has a candidate-controlled dossier plus a first grammar slice over its curated candidate layer. The next substantive task should stay inside numerals rather than reopening the already stabilized packets or broadening into larger topic families.
