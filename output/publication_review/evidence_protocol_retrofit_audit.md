...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| interrogatives | no | no | no | no | yes (`candidates_interrogatives.tsv`) | `tests/test_interrogatives_candidates.py`, `tests/test_publication_candidate_extractor.py`, plus `tests/test_grammar_integration.py` safety checks for `interrogative-hiam` | no report-level correction in this commit; the existing generated report remains the discovery source | formulaic `Bang hang hiam cih leh`, lexical/non-particle `hiam`, embedded-question material, `bangmah` / `bangin` false friends, overlap with sentence-final particles, and deferred comparison particles (`maw`, `ham`, `em`) | first analyzer-aware candidate layer and curated extractor route now exist, with accepted core `hiam`/WH rows plus explicit blocked false friends; dossier and print slices have not started yet | stabilize the candidate layer, keep the route narrow and curated, and build the dossier only after the blocked/accepted split is confirmed |
...
Interrogatives has now begun as the next narrow retrofit, but it is not complete. The new `candidates_interrogatives.tsv` file and curated extractor route keep the first-pass evidence layer deliberately small: accepted clause-final `hiam` and WH-question rows, one embedded-question row left under review, and explicit blocked controls for formulaic `Bang hang hiam cih leh`, lexical/non-interrogative `a hiam ...` sequences, and bang-family noise such as `bangmah` and `bangin`.

The topic should stay narrow at this stage. Comparison particles `maw`, `ham`, and `em` remain deferred, and the next interrogatives task should be dossier work rather than a broad search for every `hiam`, `bang`, or `kua` hit in the corpus.
...
2. **Continue the active interrogatives retrofit through the candidate-first sequence** — the candidate TSV and curated extractor route now exist, but dossier and print-slice work have not started yet.
