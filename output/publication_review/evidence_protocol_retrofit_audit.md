...
With negation now retrofitted and pronouns / clusivity now hardened at the publication layer and repaired upstream for `ko`, the remaining practical order should be:

1. **Stem alternation next** — it is again the clearest remaining packet that lacks explicit candidate rows.
...
The retrofit audit now supports a clear next step again. Demonstratives remains the protocol pilot, negation is the first hardened retrofit, pronouns / clusivity is now the second hardened retrofit with its `ko` analyzer issue addressed upstream, and the remaining backlog should continue toward the same candidate-first architecture with stem alternation next.
