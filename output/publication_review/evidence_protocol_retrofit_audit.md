...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| interrogatives | yes (`dossier_interrogatives.md`) | no | no | no | yes (`candidates_interrogatives.tsv`) | `tests/test_interrogatives_candidates.py`, `tests/test_interrogatives_dossier.py`, `tests/test_publication_candidate_extractor.py`, plus `tests/test_grammar_integration.py` safety checks for `interrogative-hiam` | no report-level correction in this commit; the existing generated report remains the discovery source | formulaic `Bang hang hiam cih leh`, lexical/non-particle `hiam`, embedded-question material, `bangmah` / `bangin` false friends, overlap with sentence-final particles, and deferred comparison particles (`maw`, `ham`, `em`) | first analyzer-aware candidate layer, curated extractor route, and candidate-controlled dossier now exist; grammar, dictionary, and review-note slices have not started yet | hold the route narrow, use the candidate TSV plus dossier as the control for the first grammar slice, and keep dictionary/review-note work for later |
...
Interrogatives has now moved from candidate-layer startup into dossier stage, but it is not complete. The new `candidates_interrogatives.tsv` file, curated extractor route, and `dossier_interrogatives.md` keep the first-pass evidence layer deliberately small: accepted clause-final `hiam` and WH-question rows, one embedded-question row left under review, and explicit blocked controls for formulaic `Bang hang hiam cih leh`, lexical/non-interrogative `a hiam ...` sequences, and bang-family noise such as `bangmah` and `bangin`.

The topic should stay narrow at this stage. Comparison particles `maw`, `ham`, and `em` remain deferred, and the next interrogatives task should be the first grammar print slice rather than a broad search for every `hiam`, `bang`, or `kua` hit in the corpus.
...
2. **Continue the active interrogatives retrofit through the candidate-first sequence** — the candidate TSV, curated extractor route, and dossier now exist, but grammar, dictionary, and review-note slice work have not started yet.
