...
Interrogatives now has a candidate-controlled dossier plus first grammar and dictionary print slices, but it is not complete. The current `candidates_interrogatives.tsv` file, curated extractor route, `dossier_interrogatives.md`, `grammar_interrogatives_print_slice.md`, and `dictionary_interrogatives_print_slice.md` keep the first-pass evidence layer deliberately small: accepted clause-final `hiam` and WH-question rows, one embedded-question row left under review, and explicit blocked controls for formulaic `Bang hang hiam cih leh`, lexical/non-interrogative `a hiam ...` sequences, and bang-family noise such as `bangmah` and `bangin`.

The topic should stay narrow at this stage. Comparison particles `maw`, `ham`, and `em` remain deferred, and the next interrogatives task should be review-note work only if the project decides that the grammar and dictionary slices are stable enough to extend.
...
2. **Continue the active interrogatives retrofit through the candidate-first sequence** — the candidate TSV, curated extractor route, dossier, and first grammar and dictionary slices now exist, but review-note work has not started yet.
3. **Keep interrogatives narrow** — stabilize core `hiam` and WH-question evidence first, keep formulaic and lexical false friends blocked, and leave `maw`, `ham`, and `em` deferred for later review.
