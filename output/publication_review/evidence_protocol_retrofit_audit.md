...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| interrogatives | yes (`dossier_interrogatives.md`) | yes (`grammar_interrogatives_print_slice.md`) | no | no | yes (`candidates_interrogatives.tsv`) | `tests/test_interrogatives_candidates.py`, `tests/test_interrogatives_dossier.py`, `tests/test_interrogatives_print_slice.py`, `tests/test_publication_candidate_extractor.py`, plus `tests/test_grammar_integration.py` safety checks for `interrogative-hiam` | no report-level correction in this commit; the existing generated report remains the discovery source | formulaic `Bang hang hiam cih leh`, lexical/non-particle `hiam`, embedded-question material, `bangmah` / `bangin` false friends, overlap with sentence-final particles, and deferred comparison particles (`maw`, `ham`, `em`) | first analyzer-aware candidate layer, curated extractor route, candidate-controlled dossier, and first grammar slice now exist; dictionary and review-note slices have not started yet | hold the route narrow, use the candidate TSV plus dossier plus grammar slice as the control for any later dictionary or review-note work |
...
Interrogatives now has both a candidate-controlled dossier and a first grammar print slice, but it is not complete. The current `candidates_interrogatives.tsv` file, curated extractor route, `dossier_interrogatives.md`, and `grammar_interrogatives_print_slice.md` keep the first-pass evidence layer deliberately small: accepted clause-final `hiam` and WH-question rows, one embedded-question row left under review, and explicit blocked controls for formulaic `Bang hang hiam cih leh`, lexical/non-interrogative `a hiam ...` sequences, and bang-family noise such as `bangmah` and `bangin`.

The topic should stay narrow at this stage. Comparison particles `maw`, `ham`, and `em` remain deferred, and the next interrogatives task should be dictionary or review-note work only if the project decides that the grammar slice is stable enough to extend.
...
2. **Continue the active interrogatives retrofit through the candidate-first sequence** — the candidate TSV, curated extractor route, dossier, and first grammar slice now exist, but dictionary and review-note slice work have not started yet.
