...
| stem alternation | yes | yes | yes | yes | yes (`candidates_stem_alternation.tsv`) | `tests/test_stem_alternation_candidates.py` plus extractor reproducibility coverage | no explicit report correction located; review notes instead describe noisy report/questionnaire layers | Form I / Form II distribution, noisy questionnaire output, lexical-family contamination (`ngai/ngaih`), caveated expansions such as `za ~ zak` and `nusia ~ nusiat` | candidate TSV added with accepted core pairs plus explicit blocked report-noise and lexical-family rows | harden the new stem candidate layer against packet prose, then move to case marking |
...
Stem alternation is a strong packet, and it now has an analyzer-aware candidate file. That retrofit preserves the packet's strongest core pairs, keeps caveated expansions such as `za ~ zak` and `nusia ~ nusiat` visible, and explicitly blocks report/questionnaire traps such as `piangsak`, `ngaihsutna`, and solitary `honkhiat`. The review notes are still right that the report layer is too uneven to auto-generate the chapter safely, so the next task is to harden this new candidate layer against the packet prose rather than to treat the topic as fully closed.
...
With negation now retrofitted, pronouns / clusivity now hardened at the publication layer and repaired upstream for `ko`, and stem alternation now given an explicit candidate TSV, the remaining practical order should be:

1. **Stem alternation hardening next** — review the new candidate TSV against the packet prose and keep noisy questionnaire/report rows explicitly blocked.
...
The retrofit audit now supports a slightly narrower next step. Demonstratives remains the protocol pilot, negation is the first hardened retrofit, pronouns / clusivity is the second hardened retrofit with its `ko` analyzer issue addressed upstream, and stem alternation now has its own explicit candidate TSV. The immediate next step is to harden that new stem candidate layer before moving on to case marking.
