...
With demonstratives, negation, pronouns / clusivity, stem alternation, and case marking now all present under the current protocol, the remaining practical order should be:
...
2. **Choose the next retrofit target from an existing grammar-report topic rather than reopening the current packets** — the decision should now be deliberate and file-backed.
3. **Prefer a narrow next topic over a broad verbal system** — interrogatives is currently the strongest next candidate because `docs/grammar/reports/09-sent-01-interrogatives.md` already exists and `tests/test_grammar_integration.py` already enforces `interrogative-hiam` safety constraints, while broad TAM and directionals remain deferred.
...
The retrofit audit now supports a clearer roadmap. Demonstratives remains the protocol pilot, negation is the first hardened retrofit, pronouns / clusivity is the second hardened retrofit with its `ko` analyzer issue addressed upstream, stem alternation is the third completed retrospective retrofit at the publication-review evidence-architecture level, and case marking is now fully aligned at the same candidate-first slice maturity level. The next substantive task should therefore be to choose a deliberately new retrofit target from the remaining existing Tedim report topics rather than to reopen the current packets.
