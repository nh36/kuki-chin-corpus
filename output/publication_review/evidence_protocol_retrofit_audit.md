...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| numerals | no | no | no | no | yes (`candidates_numerals.tsv`) | `tests/test_numerals_candidates.py`, `tests/test_publication_candidate_extractor.py` | no report-level correction in this commit; the existing generated report remains the discovery source | `kua` = nine vs `kua` = who, `khat` = one vs indefinite, numeral/classifier boundary, report-style distributive reduplication, and raw-count spillover from generated prose | first analyzer-aware candidate layer and curated extractor route now exist; the numerals retrofit has begun but is still at candidate stage | keep the route narrow, use explicit ambiguity controls for `kua` and `khat`, and draft the dossier only after the candidate layer has been reviewed |
...
3. **Continue numerals through the candidate-first sequence** — the first analyzer-aware `candidates_numerals.tsv` layer and curated extractor route now exist, but dossier and print-slice work have not started yet.
