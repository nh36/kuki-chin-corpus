...
| pronouns | no separate pronoun dossier; clusivity handled in a separate dossier plus an analyzer-quality dossier | yes | yes | yes | yes (`candidates_pronouns.tsv`) | `tests/test_pronoun_candidates.py`, `tests/test_pronoun_clusivity_docs.py`, and extractor reproducibility coverage | yes: `docs/grammar/reports/06-func-01-pronouns.md` now treats `ko/kote` as exclusive and keeps `ei/eite` under review | unresolved `ei/eite`, systematic `ko` export mismatch, possessive vs agreement prefixes, `hong-/kong-` | candidate TSV hardened at the publication layer; analyzer-quality dossier now documents that `ko` still exports as lexical `long` / `ADJ` in discourse-clear exclusive contexts | finish the narrow `ko` analyzer-quality follow-up before trusting pronoun export uncritically in later retrofits or moving on to stem alternation |
| pronoun clusivity | yes (`dossier_pronoun_clusivity.md`) | no | no | no separate review note | yes, via `candidates_pronouns.tsv` | `tests/test_pronoun_candidates.py`, `tests/test_pronoun_clusivity_docs.py`, `tests/test_pronoun_analyzer_quality.py` | yes, via the partial correction in `docs/grammar/reports/06-func-01-pronouns.md` | mixed `ei/eite`, more stable `ko/kote`, dialogue-context sensitivity, analyzer/export mismatch for `ko` | support-layer dossier now paired with a hardened shared pronoun candidate TSV and a targeted analyzer-quality diagnosis | keep `ei/eite` unresolved and treat philologically checked Tedim evidence, not Henderson alone or export labels alone, as the control while the `ko` analyzer issue is addressed |
...
However, the pronoun retrofit has now exposed a real analyzer-quality issue. The accepted `ko` candidate row is philologically secure in several addressed-dialogue contexts, yet the current export still emits lexical `long` / `ADJ` analyses for `ko`. The new analyzer-quality dossier therefore matters before another analyzer-backed retrofit treats pronoun export as automatically trustworthy.
...
The new analyzer-quality dossier adds one methodological clarification. Henderson's concord table is relevant, but it is not decisive on its own. The immediate control for pronoun-quality review is the manually checked Tedim discourse evidence, especially where speaker and addressee relations make the `ko/kote` exclusive reading secure even when the export still mislabels `ko`.
...
With negation now retrofitted and pronouns / clusivity now hardened at the publication layer, the remaining practical order should be:

1. **Small analyzer-quality follow-up on `ko` first** — the publication layer is safe, but later analyzer-backed retrofits should not inherit the current `ko = long / ADJ` mismatch without review.
2. **Stem alternation after the `ko` follow-up** — still the clearest remaining packet that lacks explicit candidate rows once the pronoun analyzer issue is fenced off.
...
The retrofit audit now supports a slightly narrower updated next step than before. Demonstratives remains the protocol pilot, negation is the first hardened retrofit, pronouns / clusivity is now the second hardened retrofit, but the newly documented `ko` export mismatch should receive a small analyzer-quality follow-up before stem alternation begins. After that, the remaining backlog should continue toward the same candidate-first architecture in the order above.
