...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| case marking | no dossier located | yes (`grammar_case_marking_print_slice.md`) | yes (`dictionary_case_markers_print_slice.md`) | yes (`review_notes_case_marking.md`) | no | no packet-specific publication-review test located; general integration and export tests touch case markers | no explicit report-level correction located during this audit | homographic `-in`, `-ah` / `-a` overgeneration, source `-pan` with conservative `-panin`, source vs relator-noun boundaries, `-tawh` accompaniment vs material/instrumental extension | existing slice family, but it predates the candidate-first workflow | next retrofit target; add an analyzer-aware candidate TSV before further substantive case-marking revision |
...
Case marking is the next retrofit target precisely because it is existing slice territory rather than a new topic. The current packet has `grammar_case_marking_print_slice.md`, `review_notes_case_marking.md`, and a dictionary slice under the older filename `dictionary_case_markers_print_slice.md`, but no standalone dossier and no `candidates_case_marking.tsv`. The next step should therefore be a candidate-first retrofit rather than immediate prose expansion. That retrofit needs to stay conservative around homographic `-in`, raw `-ah` / `-a` overgeneration, source `-pan` with conservative `-panin`, source-vs-relator-noun boundaries, and `-tawh` accompaniment vs material/instrumental extension.

## Recommended retrofit order

With demonstratives, negation, pronouns / clusivity, and stem alternation now all at maintenance or human-review status under the current protocol, the remaining practical order should be:

1. **Case marking candidate retrofit next** — retrofit the existing slice family with an analyzer-aware candidate TSV and conservative ambiguity controls.
2. **Then decide between another retrofit or a genuinely new topic** — after case marking is brought into the candidate-first workflow, reassess whether another existing slice needs the same treatment.
3. **Demonstratives, negation, pronouns / clusivity, and stem alternation remain maintenance/human-review topics for now**.

## Conclusion

The retrofit audit now supports a clearer roadmap. Demonstratives remains the protocol pilot, negation is the first hardened retrofit, pronouns / clusivity is the second hardened retrofit with its `ko` analyzer issue addressed upstream, and stem alternation is now the third completed retrospective retrofit at the publication-review evidence-architecture level. The next substantive retrofit should be case marking, because it is an existing slice family that still lacks a candidate-first evidence layer.
