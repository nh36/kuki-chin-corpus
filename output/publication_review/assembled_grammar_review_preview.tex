...
This is now a normalized publication-facing NP structure / possession
section, following the coverage-normalization standard defined in
\texttt{coverage\_normalization\_audit.md} and piloted in the numerals
and quantifiers sections. It remains controlled by candidate evidence
and explicit caveats rather than by raw generated-report counts or broad
analyzer output \citep{henderson1965, zamngaihcing2017}.
...
\tightlist
\item
  \texttt{coverage\_normalization\_audit.md}
\item
  \texttt{candidates\_np\_possession.tsv}
\item
  \texttt{dossier\_np\_possession.md}
\item
  \texttt{review\_notes\_np\_possession.md}
\item
  \texttt{docs/grammar/reports/03-noun-06-np-structure.md}
\item
  \texttt{docs/grammar/reports/04-np-07-possession.md}
\item
  \texttt{examples\_np\_possession\_normalization.tsv}
\end{itemize}

The numerals and quantifiers sections already carry the fuller analysis
of numeral and quantifier systems. This section reuses those checked
examples only to support modest noun-phrase structure claims.
...
proximal form precedes the noun, as in \tdim{hih mite}. Numerals and
several quantifier-like forms follow the noun in the cleanest current
examples, as seen in \tdim{mi khat}, \tdim{ni li}, \tdim{mi khempeuh},
\tdim{mi pawlkhat}, and \tdim{mi tampi}. The present section therefore
supports a modest contrast between demonstrative-before-noun structure
and noun-plus-postnominal numeral or quantifier structure, while leaving
broader NP templating for later review.

Possession is now visible enough to include as part of the same
publication-facing section, but it remains less normalized than the
modifier-order material. The safest checked rows show
possessor-before-possessed structure in compact noun phrases such as
\tdim{na pa' inn-ah} and \tdim{a zi' min}, while the analysis of
possessive prefixes, apostrophe marking, genitive structure, and relator
or case overlap still requires explicit caveats.
...
in the current pass because the Gospel comparanda were less clean under
present candidate control.
...
description for the current packet. That is a narrower and safer claim
...
clean as a simple adnominal demonstrative anchor. For print purposes,
...
\glt 'in thy father's house' (Genesis 24:23)
\end{exe}
...
\glt 'his wife's name' (Genesis 3:20)
\end{exe}
...
the packet anchors \tdim{ka pa}, \tdim{Topa' inn}, and \tdim{a pa' inn}.
...
No equally clean Gospel possession row was found for this pass under
current candidate discipline. The possession subsection therefore
remains OT-led even though the section as a whole includes
Gospel-balanced NP-order examples.
...
\item
  ...
  publication prose here.
\end{itemize}

\subsubsection{Summary}\label{summary}

The NP structure / possession section can now support a modest
publication-facing description of noun phrase order: the cleanest
current evidence points to demonstrative-before-noun structure alongside
noun-plus-postnominal numeral and quantifier patterns. Possession is now
visible enough to include with formal examples, but it remains a
cautious subsection whose broader paradigm, marking analysis, and
interface with pronouns, genitive structure, and relators are still
explicitly deferred.
...
This is now a normalized publication-facing noun domain section,
following the coverage-normalization standard defined in
\texttt{coverage\_normalization\_audit.md} and piloted in the numerals,
quantifiers, and NP structure / possession sections. It remains
controlled by candidate evidence and explicit caveats rather than by raw
generated-report counts or broad analyzer output
...
\item
  \texttt{coverage\_normalization\_audit.md}
\item
  \texttt{candidates\_noun\_domain.tsv}
\item
  \texttt{dossier\_noun\_domain.md}
\item
  \texttt{review\_notes\_noun\_domain.md}
\item
  \texttt{docs/grammar/GRAMMAR\_SOURCE\_INVENTORY.md}
\item
...
\item
  \texttt{docs/grammar/reports/03-noun-04-plural.md}
\item
  \texttt{docs/grammar/reports/03-noun-05-nominalization.md}
\item
  \texttt{examples\_noun\_domain\_normalization.tsv}
\end{itemize}

The normalized NP, numerals, and quantifiers sections already carry the
fuller discussion of noun phrase order, numeral formation, and
quantifier semantics. The present section reuses those checked rows only
to show which lexical noun anchors are currently safe to describe in
...
description. Simple lexical nouns such as \tdim{gam} and \tdim{aksi} are
safely attested, and plural marking with \tdim{-te} is printable for
controlled anchors such as \tdim{aksi-te} and \tdim{mite}. Human noun
\tdim{mi} is also a stable lexical head in larger noun phrases, as seen
in rows such as \tdim{mi khat}, \tdim{mi khempeuh}, \tdim{mi pawlkhat},
and \tdim{mi tampi}, while nonhuman nouns such as \tdim{ni} and
\tdim{kum} behave similarly in counted phrases.

This is enough to describe a small publication-facing noun inventory, to
note a cautious \tdim{-te} plural pattern, and to show that lexical
nouns remain visible as heads inside demonstrative, numeral, and
quantifier phrases. It is not yet enough for a full noun-domain chapter,
because compounds, proper names, classifier-like nouns, and nominalized
nouns still require more boundary control.
...
however, remain OT-led in the current pass.
...
\bottomrule\noalign{}
\endlastfoot
\tdim{gam} & simple common noun `land / country' & \tdim{gam sung} &
print-ready & semantic range varies across geographic and political
contexts \\
\tdim{aksi} & simple common noun `star' & \tdim{ama aksi} & print-usable
with caveat & current Gospel anchor occurs inside a possessed phrase \\
\tdim{aksi-te} & noun with \tdim{-te} plural marking & \tdim{aksi-te} &
print-ready & does not yet settle the full behavior of plural /
collective marking \\
\tdim{mi} & human common noun `person' & \tdim{mi khat},
\tdim{mi khempeuh} & print-ready & overlaps with quantified and counted
NP structure discussed elsewhere \\
\tdim{mi-te} / \tdim{mite} & human plural noun & \tdim{hih mite} &
print-ready & demonstrative and NP-order analysis belongs to other
sections \\
\tdim{ni} & noun head in counted phrase & \tdim{ni li} & print-ready &
numeral semantics belong to the numerals section \\
\tdim{kum} & noun head in counted phrase & \tdim{kum sawm le nih} &
print-ready & compound numeral analysis belongs to the numerals
section \\
\tdim{mi-nam} / \tdim{minam} & transparent compound noun &
\tdim{minam khat} & print-usable with caveat & compound transparency and
lexicalization remain under review \\
\tdim{Abraham} & proper name used inside a larger noun phrase &
\tdim{Abraham' suan David} & print-usable with caveat & does not yet
...
enough to show that the noun packet is not limited to derived forms or
...
a modest claim that \tdim{-te} marks plurality on nouns in
publication-facing examples.
...
form \tdim{mite} is equally secure in the current packet.
...
nominal, but the current packet is not yet strong enough to generalize
...
\item
  ...
  promoted into publication prose.
\item
  Raw report-only noun lists are not treated as grammar facts without
  checked candidate control.
\end{itemize}

\subsubsection{Summary}\label{summary-1}

The noun domain can now support a fuller publication-facing section than
the earlier narrow packet allowed. Current evidence safely identifies
simple noun stems such as \tdim{gam} and \tdim{aksi}, a cautious
\tdim{-te} plural pattern visible in \tdim{aksi-te} and \tdim{mite},
stable human noun heads such as \tdim{mi}, and lexical nouns that remain
visible inside counted and quantified noun phrases. Compounds, proper
nouns, and nominalized nouns are now better framed as explicit boundary
material rather than being mixed into an under-controlled noun list
\citep{henderson1965, zamngaihcing2017}.
...
This is now a normalized publication-facing case marking section,
following the coverage-normalization standard defined in
\texttt{coverage\_normalization\_audit.md} and piloted in the numerals,
quantifiers, NP structure / possession, and noun domain sections. It
remains controlled by candidate evidence and explicit caveats rather
than by raw generated-report counts or broad analyzer output
\citep{henderson1965, zamngaihcing2017}.
...
\item
  \texttt{examples\_case\_marking\_normalization.tsv}
\item
  \texttt{docs/grammar/GRAMMAR\_SOURCE\_INVENTORY.md}
\item
  \texttt{docs/grammar/morphemes/02-case-markers.md}
\item
  \texttt{docs/grammar/lit-reviews/03-noun-05-postpositions-lit.md}
\item
  \texttt{docs/grammar/reports/03-noun-04-relators.md}
\item
  \texttt{docs/grammar/reports/03-noun-05-postpositions.md}
\item
  \texttt{docs/grammar/reports/03-noun-06-np-structure.md}
\item
  \texttt{docs/grammar/reports/04-np-07-possession.md}
...
\item
  \texttt{output/grammar/case\_marking\_report.md}
\end{itemize}

The normalized NP structure / possession, noun domain, relators /
postpositions, and transitivity sections already carry fuller discussion
of possessor structure, spatial relators, and clause-level valency. The
present section only states the case-like patterns that are now safe to
print, marks where those other sections begin to take over, and keeps
the unresolved edges explicit.
...
postnominal case-like marking in Tedim. The clearest publication-facing
claims concern locative or goal-like \tdim{-ah} and clause-level
agentive or ergative \tdim{-in}. The same packet also supports more
cautious discussion of source marking with \tdim{-pan} and
\tdim{-panin}, plus accompaniment or material-extension uses of
\tdim{-tawh}, but those markers remain more boundary-heavy because they
overlap with relator nouns, postpositions, or broader semantic
extension.
...
\tdim{inn-ah} & print-ready & must stay distinct from deferred
tone-sensitive \tdim{-a} and from relator-hosted spatial phrases \\
\tdim{-in} & clause-level ergative or agentive marking & \tdim{Kain in},
\tdim{Herod in} & print-ready & raw \tdim{-in} extraction overgenerates
forms such as \tdim{ciangin} \\
possessor phrase plus \tdim{-ah} & possessed NP closed by case-like
marking & \tdim{na pa' inn-ah} & print-usable with caveat & does not
settle the apostrophe or genitive analysis \\
\tdim{-pan} & source marking, often on a relator host & \tdim{lakpan} &
print-usable with caveat & current clean evidence is relator-hosted
rather than a broad bare-suffix paradigm \\
\tdim{-panin} & source or departure marking & \tdim{inn panin} &
print-usable with caveat & internal structure remains under review \\
\tdim{-tawh} & accompaniment, with material or instrumental extension &
\tdim{kei tawh}, \tdim{leivui tawh} & print-usable with caveat &
...
print-usable with caveat & belongs jointly with the relators /
...
case marker at the right edge of the noun phrase. The primary
candidate-backed control is plain noun-plus-locative \tdim{khua-ah},
while the new source-balance supplement adds a compact Gospel goal-like
example with \tdim{keima inn-ah}. This is enough to describe \tdim{-ah}
as marking location and destination-like relations without forcing a
sharper split among locative, allative, and general oblique labels than
the current packet can support.
...
material into \tdim{-ah}. The source inventory and case dossier both
keep tone-sensitive or otherwise ambiguous \tdim{-a} questions
unresolved, so the publication-facing description remains narrower than
a full locative or allative chapter.

\begin{exe}
\ex \label{ex:case-ah-khua}
\gll \tdimword{khùa-àh} \\
     town-\textsc{loc} \\
\glt 'in the town' (Genesis 11:28)
\end{exe}
...
publication prose, but not yet enough to decide whether every \tdim{-ah}
token should be described as specifically locative, allative, or more
generally oblique.

\subsubsection{Agentive, ergative, or instrumental marking with
-in}\label{agentive-ergative-or-instrumental-marking-with--in}

The present packet supports an agentive or ergative analysis of
\tdim{-in} more clearly than it supports any broader instrumental
analysis. Genesis 4:3 remains the primary candidate-backed anchor
because \tdim{Kain in} is the cleanest controlled row. The new
supplement adds Matthew 2:4 \tdim{Herod in} as a manually reviewed
Gospel support example, which helps show that the pattern is not
confined to one OT narrative window.
...
\glt 'Cain as transitive subject' (Genesis 4:3)
\end{exe}
...
\glt 'Herod as transitive subject' (Matthew 2:4)
\end{exe}
...
The packet still retains a small set of other controlled case-like
...
Likewise, \tdim{-tawh} remains part of the packet because
...
\glt 'in the city of David' (Luke 2:11)
\end{exe}
...
collapsed into case marking without candidate control. The best current
solution is to let the two sections meet at the boundary rather than
...
The present section therefore cross-references the transitivity packet
...
The case-marking section can now support a fuller publication-facing
description than the earlier narrow packet allowed. Current evidence
safely supports clause-level \tdim{-in}, locative or goal-like
\tdim{-ah}, a boundary note on possessed noun phrases such as
\tdim{na pa' inn-ah}, and a controlled interface with relator-hosted
forms such as \tdim{David khuapi sungah} and \tdim{lakpan}. Source
marking with \tdim{-pan} and \tdim{-panin}, along with \tdim{-tawh},
remains usable but secondary and caveated. The result is a real grammar
section with explicit limits, not a claim that the full Tedim case
...
This is now a normalized publication-facing numerals section, not just
the first narrow slice. It is controlled by
\texttt{candidates\_numerals.tsv} and \texttt{dossier\_numerals.md}, and
it is additionally checked against \texttt{review\_notes\_numerals.md},
\texttt{coverage\_normalization\_audit.md}, and
\texttt{examples\_numerals\_normalization.tsv}.

The section still keeps candidate discipline. Printed claims and formal
examples come from candidate evidence in
\texttt{candidates\_numerals.tsv} or from newly checked normalization
examples in \texttt{examples\_numerals\_normalization.tsv}, not from raw
generated-report counts and not from broad string searches over every
numeral-looking form. The separate dictionary print slice still exists,
but this section now aims to read like a real grammar section rather
than a packet-status note.
...
system, with \tdim{sawm} as the ten base and \tdim{za} as the next
larger base above the first two-digit range
\citep{zamngaihcing2017, henderson1965}. The Bible-backed review packet
confirms that basic cardinals, compound tens, larger-number expressions,
\tdim{-na} ordinals, and at least one occurrence-counting expression are
all securely part of the current publication-facing section.
...
output without candidate control.
...
section. It is publication-facing orientation rather than a raw
frequency table: the point is to show the basic system, while the formal
examples below remain restricted to candidate-controlled or newly
checked normalization rows.
...
1,000 & \tdim{sing} & thousand base; not fully normalized in this
pass \\
10,000 & \tdim{tul} & larger unit recognized in the report/literature
layer; not fully normalized in this pass \\
\end{longtable}
}

The table does not by itself authorize every form for immediate
print-heavy exposition. It gives the normalized section a visible
inventory, while the examples below keep the actual printed claims small
and checked.
...
numerals through decimal composition, but the publication-facing section
should phrase that conservatively. The report and literature support a
decimal structure \citep{zamngaihcing2017, henderson1965}, while the
current packet gives two especially useful checked examples: one Old
Testament compound-ten anchor and one Gospel counted-noun expression
with a compound numeral.
...
combinations, but the present normalized section only promotes the best
checked examples.
...
The normalized section therefore keeps the row with its analyzer caveat
visible instead of pretending that the export is already perfect.
...
\gll \tdimword{nìh-ná} \\
     two-\textsc{nmlz} \\
\glt 'second' (Genesis 7:11)
\end{exe}

\tdim{Nihna} is the current print-ready ordinal row. The dossier already
notes that \tdim{pos\_span = N} in the export; that is a label caveat,
not a reason to reject the ordinal analysis.
...
`first', but the present section does not yet promote it as the
normalized ordinal anchor because the current candidate-controlled
packet is still built around \tdim{nihna}, not around a full ordinal
paradigm or a full contrast between \tdim{masa} and \tdim{khatna}-type
forms.
...
patterns are securely attested. The normalized section now keeps both
Old Testament and Gospel evidence visible for that pattern.

\begin{exe}
\ex \label{ex:num-kum-nih}
\gll \tdimword{kúm} \tdimword{nìh} \\
     year two \\
\glt 'two years' (Genesis 11:10)
\end{exe}
...
These are clean counted-noun examples, and they justify a real
publication-facing statement that noun-plus-numeral order is well
supported. The normalized section should still avoid overclaiming a
complete typology of numeral placement. The broader report layer
contains more patterns, and the literature discusses wider numeral
syntax, but the checked publication-facing section should stop at what
the current examples support directly.
...
expressions, but the current normalized section should stay selective.
Only the rows that are candidate-controlled or newly checked for this
pilot should be promoted.
...
\gll \tdimword{mì} \tdimword{khàt} \\
     person one \\
\glt 'a man' / 'one person' (Genesis 32:24)
\end{exe}
...
\bottomrule\noalign{}
\endlastfoot
\tdim{mi khat} & print-usable with caveat & numeral/indefinite boundary,
not a simple classifier row \\
\tdim{sawmvei} & print-usable with caveat & clear occurrence-counting
expression \\
\tdim{pa}, \tdim{nu}, \tdim{zat}, \tdim{tei} & deferred & promising
report/literature material, but not normalized in this pass \\
\end{longtable}
}
...
print-ready because the current analyzer/candidate layer still does not
support the repeated span in the key Genesis 7:2 row. The generated
...
disciplined. The normalized section can say that distributive
reduplication is a promising area in the report and literature, but it
should still defer \tdim{sagih sagih} until the analyzer-backed evidence
is clean enough to promote.
...
contexts \\
\tdim{khat} & basic cardinal `one' & indefinite-like readings such as
\tdim{mi khat} & keep explicit boundary notes; do not overgeneralize
article-like use \\
\end{longtable}
}
...
The normalized numerals section now supports a genuine
publication-facing description. Tedim has a decimal numeral system with
basic cardinals, compound tens, larger bases such as \tdim{za},
\tdim{sing}, and \tdim{tul}, \tdim{-na} ordinals, noun-plus-numeral
counting phrases, and at least one compact occurrence-counting
expression. The section now includes multiple formal examples, a visible
inventory table, and both Old Testament and Gospel evidence where the
checked material allows it.
...
This is now a normalized publication-facing quantifiers section,
following the coverage-normalization standard defined in
\texttt{coverage\_normalization\_audit.md} and piloted in
\texttt{grammar\_numerals\_print\_slice.md}.

The section remains controlled by \texttt{candidates\_quantifiers.tsv},
\texttt{dossier\_quantifiers.md},
\texttt{review\_notes\_quantifiers.md},
\texttt{docs/grammar/reports/06-func-05-quantifiers.md}, and the checked
normalization supplement
\texttt{examples\_quantifiers\_normalization.tsv}. Example selection
follows the audit standard: example quality comes first, source balance
comes second.

The present section therefore keeps candidate evidence and explicit
caveats central. It does not claim that the whole quantifier system is
finished, and it does not promote raw generated-report counts as grammar
facts.

\subsubsection{Overview of quantification in
Tedim}\label{overview-of-quantification-in-tedim}

The current packet supports a modest but real publication-facing account
of quantification in Tedim. The safest core anchor is universal or total
\tdim{khempeuh}. Existential or indefinite-like quantification is
visible through \tdim{pawlkhat}, but that material still carries a
partitive or alternative-grouping caveat. \tdim{Khat} remains boundary
evidence shared with numerals rather than an uncomplicated quantifier
anchor. Negative-quantifier work is currently safest with \tdim{kuamah}
and \tdim{bangmah} only where clear clause-level negation licenses them.
Quantity-related material is also visible through \tdim{tampi tak} and
noun-phrase \tdim{mi tampi}, but those rows still border on broader
degree or modification work rather than constituting a full independent
...
The present section therefore keeps five safe publication-facing claims
in view. First, \tdim{khempeuh} supports a modest universal or
total-quantifier generalization. Second, \tdim{pawlkhat} supports
existential or indefinite-like readings with an explicit grouping
caveat. Third, \tdim{kuamah} and \tdim{bangmah} remain usable only in
negative-licensed clauses. Fourth, noun phrases such as
\tdim{mi khempeuh}, \tdim{mi pawlkhat}, and \tdim{mi tampi} support only
a modest noun-plus-quantifier discussion. Fifth, \tdim{peuhpeuh},
\tdim{tawm}, \tdim{zaw}, and \tdim{mahmah} remain visible as boundary
material without broadening the section into a full free-choice,
low-quantity, comparative, or intensifier chapter.
...
discuss in the normalized section.
...
export-noisy & deferred & current analyzer gloss is too noisy for print
use \\
\tdim{zaw} & comparative edge row & visible only as boundary material &
edge row only & comparison is not normalized as part of the core
quantifier chapter \\
\tdim{mahmah} & intensifier edge row & visible only as boundary material
& edge row only & intensification belongs to broader modification work,
...
total-quantifier anchor. The original packet anchor is still Genesis
2:1, which gives a well-scoped nominal expression and keeps the printed
claim conservative.
...
The normalized section also adds a clean Gospel counterpart so that the
section does not become another Genesis-heavy chapter.

\begin{exe}
\ex \label{ex:quant-mi-khempeuh}
\gll \tdimword{mì} \tdimword{khèmpeuh} \\
     person all \\
\glt 'all people' / 'everyone' (Luke 2:1)
\end{exe}
...
currently the best publication-facing anchor. Matthew 2:3
\tdim{Jerusalem khuami khempeuh} points in the same direction and
supports the same noun-plus-quantifier pattern, but the printed section
does not need to turn that into a broad universal/distributive system or
a report-driven frequency claim.
...
\tdim{pawlkhat}, but the packet's original Genesis 32:8 control still
has to be read as partitive or alternative-grouping evidence rather than
as an uncomplicated bare \tdim{some}.

For the normalized section, a checked Gospel noun-phrase example helps
show the construction more clearly in print:

\begin{exe}
\ex \label{ex:quant-mi-pawlkhat}
\gll \tdimword{mì} \tdimword{pàwl-khàt} \\
     person some-one \\
\glt 'some people' (Matthew 2:1)
\end{exe}

This Gospel row is useful because it makes the noun-phrase behavior
clearer, but the Genesis 32:8 packet anchor still controls the
...
\glt 'nobody can do anything' (John 3:27)
\end{exe}
...
row \tdim{bangmah om lo hi} remains the compact candidate-backed
nonhuman anchor behind this subsection, but John 3:27 gives the
normalized section a good Gospel example without loosening the negation
caveat.
...
the quantifiers packet from absorbing every \tdim{bangmah} token outside
clear negative licensing.
...
\gll \tdimword{mì} \tdimword{tampi} \\
     person many \\
\glt 'many people' (Mark 6:34)
\end{exe}

This is where the section can integrate quantity material without
broadening uncontrollably. The original packet's \tdim{tampi tak} row is
still the main quantity/degree anchor, but \tdim{mi tampi} gives the
normalized section a cleaner noun-phrase example. The printed claim
should therefore stay narrow: Tedim clearly allows noun-plus-quantifier
or noun-plus-quantity combinations in at least some checked rows, but
the present packet does not yet settle a full NP-ordering typology.
...
\bottomrule\noalign{}
\endlastfoot
numeral overlap with \tdim{khat} & print-usable with caveat &
\tdim{mi khat} is shared boundary evidence and should not be detached
from the numerals discussion \\
negative quantifier overlap & print-usable with caveat & \tdim{kuamah}
and \tdim{bangmah} are safe only with explicit negation \\
...
\tdim{tampi tak} versus broader degree work & print-usable with caveat &
...
\tdim{bangmah}, so the normalized section now has genuine Old Testament
and Gospel coverage. The same search did not yield a cleaner Gospel
replacement for the classic \tdim{mi khat} boundary row, and Gospel
\tdim{peuhpeuh} material remained too free-choice or discourse-heavy to
promote as settled core quantifier evidence.

\subsubsection{Summary}\label{summary-4}

The quantifiers section now reads as a real publication-facing grammar
section rather than a narrow packet note. \tdim{Khempeuh} remains the
clearest universal anchor; \tdim{pawlkhat} remains useful existential or
indefinite-like evidence with a grouping caveat; \tdim{khat} remains
explicit numeral/indefinite boundary material; \tdim{kuamah} and
\tdim{bangmah} remain tied to clear negative licensing; and \tdim{tampi}
material now supports a modest noun-phrase discussion without widening
into a broad modification chapter.
...
degree/intensifier/comparative chapter. It keeps candidate evidence and
