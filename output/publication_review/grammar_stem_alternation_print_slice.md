...
Within this wider field, `za ~ zak` is the strongest additional pair safe enough to mention in the grammar prose now. By contrast, `thei ~ theih` and `piang ~ pian` should remain explicitly caveated: the former is real but still heavily tied to modal, purposive, and nominalized uses, while the latter is real but shows the Form II side mostly through `pianna` and `a pian nadingin` rather than through a neat bare finite paradigm.
...
The expanded coverage table makes the current editorial position more honest about that wider evidence. `za ~ zak` is now visible as a print-usable pair with caveat, `nusia ~ nusiat` is acknowledged as a promising next-stage pair, `thei ~ theih` and `piang ~ pian` remain clearly provisional, and `ngai ~ ngaih`, `honkhia ~ honkhiat`, and `hu ~ huh` remain outside simple print treatment for now.
