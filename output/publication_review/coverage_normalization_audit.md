...
| Reduplication | Chapter 4: Reduplication | `07-deriv-02-reduplication.md` | Yes | Yes | Yes | 6 | normalized section now surfaces OT/Gospel formal examples for `mahmah`, `taktak`, and secondary `peuhpeuh` | No | normalized section | core intensifier claim is now explicit; syntactic, TAM/VP, expressive, and report-only rows remain boundary-controlled | Medium | keep stable and add only cleaner boundary promotions when candidate-backed evidence improves |
