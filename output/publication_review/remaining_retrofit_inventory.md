...
| Topic | Publication-review grammar slice | Dictionary slice | Review notes | Dossier | Candidate TSV | Extractor support | File-backed evidence | Known ambiguity risks | Recommendation |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| interrogatives | no | no | no | no | yes (`candidates_interrogatives.tsv`) | yes (`extract_candidates.py`) | `output/publication_review/candidates_interrogatives.tsv`; `docs/grammar/reports/09-sent-01-interrogatives.md`; `docs/grammar/reports/10-disc-01-sentence-final.md`; `tests/test_interrogatives_candidates.py`; `tests/test_grammar_integration.py` (`interrogative-hiam`) | formulaic `Bang hang hiam ci`, lexical/non-particle `hiam`, embedded-question material, `bangmah` / `bangin` false friends, overlap with sentence-final particles, comparison particles deferred | active retrofit; stabilize candidate layer, then build dossier |
...
Demonstratives, negation, pronouns / clusivity, stem alternation, and case marking remain the completed publication-review packets at the current candidate-first level. Interrogatives is now the one active next retrofit topic rather than a future inventory item.

The next substantive task should stay inside interrogatives: use the new `candidates_interrogatives.tsv` layer to draft a dossier before any grammar or dictionary slice work begins. Broad TAM, directionals, chrestomathy, Mizo/lus, and other non-Tedim or non-slice work should remain deferred until they are chosen intentionally as a new scope.
