...
from analyze_morphemes import analyze_word, PRONOMINAL_CONCORD, OBJECT_PREFIXES
from report_utils import (BOOK_NAMES, GOSPEL_CODES, format_reference,
                          load_bible, load_kjv, find_diverse_examples, format_example)
...
def generate_report():
    ...
    report.append("")
    examples = find_diverse_examples(verses, kjv, lambda w: w.startswith('ka'), limit=3)
    ...
    report.append("### a- (3SG)")
    report.append("")
    examples = find_diverse_examples(verses, kjv, lambda w: w.startswith('a') and len(w) > 2, limit=3)
    for ref, text, word, analysis, kjv_text in examples:
        report.extend(format_example(ref, text, word, analysis, kjv_text))
    ...
    report.append("")
    examples = find_diverse_examples(verses, kjv, lambda w: w.startswith('hong'), limit=3)
    for ref, text, word, analysis, kjv_text in examples:
        report.extend(format_example(ref, text, word, analysis, kjv_text))
