...
- Pronominal prefixes / agreement / object-prefix systems now has its first candidate-first scoping packet at `output/publication_review/candidates_prefix_agreement.tsv` plus `output/publication_review/dossier_prefix_agreement_scope.md`; the packet stays narrow around `kanei` and `kainn` as the clearest agreement-versus-possession routing anchors, keeps `hongmu` and `kongmu` explicit as object-prefix or inverse-like material, and treats `ipai`, `kipan`, and broader `a-`/`ki-` issues as boundary or deferred material rather than reopening the completed pronouns/clusivity or derivation packets.
