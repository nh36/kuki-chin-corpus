...
- NP structure / possession now has a first candidate-scoping packet at `output/publication_review/candidates_np_possession.tsv` plus `output/publication_review/dossier_np_possession_scope.md`. The current packet keeps `hih mite`, `mi khat`, and `mi khempeuh` as the cleanest NP-order anchors, keeps `ka pa`, `Topa' inn`, and `a pa' inn` visible for possession and possessor-possessed structure, keeps `Topa' tungah` and `ka suahna leitang` explicit as case/relator and nominalization boundary rows, and recommends a narrow basic NP-order grammar slice as the safest next print-facing step rather than a broad possession or noun-phrase chapter.
