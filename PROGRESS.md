...
- VP structure / suffix stacking now has its first candidate-first scoping packet at `output/publication_review/candidates_vp_structure_stacking.tsv` plus `output/publication_review/dossier_vp_structure_stacking_scope.md`; the packet stays small and boundary-controlled, treating `bawlzoding` as the clearest currently missing stack while keeping `khia-ta`, `khiathei ding om lo`, `dingin`, `ciahsakkik`, `bawlsakthei`, and `paikhiatsak` explicit as overlap or deferred material rather than first-pass print prose.
...
4. [ ] Keep VP structure / suffix stacking limited to the candidate TSV plus scoping dossier until a later commit explicitly drafts a narrow grammar print slice.
