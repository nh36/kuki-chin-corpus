...
- Broad TAM / aspect / modal now has its first narrow dictionary print slice as well as its grammar slice: `output/publication_review/candidates_tam.tsv`, `output/publication_review/dossier_tam_scope.md`, `output/publication_review/grammar_tam_print_slice.md`, and `output/publication_review/dictionary_tam_print_slice.md` now keep the print-facing TAM packet limited to compact suffixal anchors such as `-ngei`, `-gige`, `-zel`, `-ta`, `-zo`, `-kik`, `-ding`, and `-thei` while keeping clause-bound, negation-overlap, sentence-final, directional, and broader VP-slot stacks out of the first slice pair.
...
2. [ ] Keep broad TAM / aspect / modal limited to the candidate/scoping packet plus the narrow grammar and dictionary slices until a later commit explicitly drafts the TAM review notes.
