...
- Pronominal prefixes / agreement / object-prefix systems now also has a first narrow grammar print slice at `output/publication_review/grammar_prefix_agreement_print_slice.md`. The current print-facing claim stays tightly limited to agreement-versus-possession routing: `kanei` is the verbal agreement anchor (`ka-nei`, `1SG-have`), `kainn` is the nominal possessive-routing anchor (`ka-inn`, `1SG.POSS-house`), `tests/test_prefix_agr_poss.py` remains the regression control, and `ainn`, `ipai`, `hongmu`, `kongmu`, `kipan`, apostrophe possession, and broader possessor syntax remain boundary or deferred material rather than being widened into a full agreement, possession, or inverse chapter.
