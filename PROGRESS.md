...
- Broad TAM / aspect / modal now has its first narrow grammar print slice: `output/publication_review/candidates_tam.tsv` and `output/publication_review/dossier_tam_scope.md` remain the controlling evidence layer, and `output/publication_review/grammar_tam_print_slice.md` now limits the print-facing TAM prose to compact suffixal anchors such as `-ngei`, `-gige`, `-zel`, `-ta`, `-zo`, `-kik`, `-ding`, and `-thei` while keeping clause-bound, negation-overlap, sentence-final, directional, and broader VP-slot stacks out of the first slice.
...
2. [ ] Keep broad TAM / aspect / modal limited to the candidate/scoping packet plus the new narrow grammar slice until a later commit explicitly drafts the TAM dictionary slice.
