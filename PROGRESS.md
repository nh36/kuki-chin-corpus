...
- Directionals has now begun as the explicitly chosen next candidate-first grammar packet: `output/publication_review/candidates_directionals.tsv` now exists, `scripts/publication_review/extract_candidates.py` now supports the curated `directionals` route, and `output/publication_review/dossier_directionals.md` now records the first candidate-controlled interpretation without starting grammar, dictionary, or review-note print slices. The packet keeps `-khia`, `-khiat`, `-toh`, `-lam`, `-sawn`, and `-suk` construction-controlled while leaving `-lut`, `-phei`, `-cip`, and `-tang` deferred or analyzer-noise-bound where no clean print-safe anchor is yet selected.
