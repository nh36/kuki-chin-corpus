...
- Clause linkage now has its first candidate-first scoping packet at `output/publication_review/candidates_clause_linkage.tsv` plus `output/publication_review/dossier_clause_linkage_scope.md`. The current packet keeps `ciangin` as the clearest subordination anchor, keeps `dingin` explicit as a clause-bound purposive or irrealis overlap row, keeps `VERB-in` and `ahih ciangin` explicit for switch-reference evidence, keeps `a bawl mi` and `omna` visible as relative-clause evidence, and leaves `muhna-ah`, `leh`, and broader clause-linkage inventory as boundary or deferred material rather than forcing an early complex-sentence grammar slice.
