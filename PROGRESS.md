...
- Relators / postpositions now has an initial candidate-first scoping layer: `output/publication_review/candidates_relators_postpositions.tsv` and `output/publication_review/dossier_relators_postpositions_scope.md` now test whether a separate relators/postpositions packet can stay narrow while keeping `kiang`, `lak`, `sung`, `tung`, `pualam`, `nuai`, `mai`, `pan`, `panin`, `tawh`, and `tawhin` explicitly boundary-controlled against the existing case-marking packet rather than turning into a broad rewrite of case marking.
...
3. [ ] Keep relators / postpositions limited to the candidate TSV plus scoping dossier until a later commit explicitly drafts a narrow, case-boundary-controlled grammar slice.
