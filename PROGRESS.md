...
- A report-to-grammar coverage reconciliation now exists at `output/publication_review/report_to_grammar_coverage_reconciliation.md`; after relative-clause handoff, it now treats relative clauses as the most recent completed target, records that no new substantive target is currently selected, and keeps human review as the next substantive action unless Nathan explicitly chooses a new scope.
- The first dedicated `ki-` reflexive/reciprocal/middle candidate-first packet has now been started, with a new candidate layer, a first grammar-facing print slice, and review notes.
- The verbal `-pih` comitative applicative packet has now been started and lifted to a first grammar-facing print slice.
- The `-pih` packet now includes an empirical stem-alternation diagnostic (`output/publication_review/pih_stem_alternation_diagnostic.md`) and a stabilized stem-diagnostic status layer in `candidates_pih_comitative_applicative.tsv`.
- The hong / kong object-prefix or inverse-like packet has now been started and lifted to a first grammar-facing print slice.
- The hong / kong packet now includes a person-configuration diagnostic (`output/publication_review/hong_kong_person_configuration_diagnostic.md`) with harmonized Matthew 25:37 glossing across the hong/kong and prefix/agreement slices.
- The switch-reference / same-subject-different-subject clause-linkage packet has now been started and lifted to a first grammar-facing print slice, with secure same-subject anchors and cautious different-subject support.
- The same-subject/different-subject clause-linkage packet has now been stabilized for example display and `-in` disambiguation, with Genesis 11:4 trimmed and Deuteronomy 10:20 kept justified in full.
- The relative-clause / relative-like nominal-modifier packet has now been started, lifted, and stabilized at first-slice maturity for example verification and `mi`/nominalization boundary control.
...
- A compact human-review handoff now exists at `output/publication_review/human_review_handoff.md`; it now includes the relative-clause / relative-like nominal-modifier packet as review-ready at first-slice maturity, and it remains a reviewer-orientation index only (not a new linguistic packet).
- The report-to-grammar coverage reconciliation has now been updated after that handoff, and it records that no new substantive target is currently selected and that the next substantive action is human review unless Nathan explicitly chooses a new scope.
...
- The phonology/tone packet has been started and lifted to a cautious first grammar-facing source-alignment slice.
- The phonology/tone Bible-attested examples have now been empirically verified in `output/publication_review/phonology_tone_bible_example_verification.md`, and the evidence classifications are hardened so `thei / -thei` stays near-minimal, `hi / hi` stays homographic lexical/grammatical, `ta / -ta` stays supporting-only with explicit `nungta` morphology caveating, and unresolved `-a` remains blocked.
...
5. [ ] Keep clause linkage stable at the current temporal-subordination slice maturity level and limit edits to reviewer-identified corrections or an explicitly chosen new narrow scope.
6. [ ] Keep same-subject and different-subject clause linkage stabilized for example display and `-in` disambiguation, ready for human review at the current first-slice maturity level.
7. [ ] Keep relative-clause / relative-like nominal-modifier packet stabilized for example verification and `mi`/nominalization boundary control, ready for human review at the current first-slice maturity level.
8. [ ] Keep no new substantive target selected by default; the next substantive action is human review of the completed packet set unless Nathan explicitly chooses a new scope.
9. [ ] Keep chrestomathy, Mizo/lus, and the other Kuki-Chin languages deferred until they are explicitly chosen as a new scope.
