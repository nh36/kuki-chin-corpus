...
- An assembled review preview now exists at `output/publication_review/assembled_grammar_review_preview.md`. It is explicitly a Markdown-only review preview assembled from first-pass publication-review slices rather than a finished grammar or final PDF, and it keeps current narrow anchors such as `bawlzoding`, `-sak`, `kanei / kainn`, `ciangin`, `-na / bawlna`, `hih mite`, `mi khat`, `mi khempeuh`, `gam`, `aksi / aksi-te`, `mahmah / taktak` with secondary `peuhpeuh`, and `sih / suak` versus `hawl / en`, while marking phonology/tone, verb paradigms, broader discourse, and analyzer-gap topics as explicit major gaps.
