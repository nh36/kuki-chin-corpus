...
- The quantifiers packet is now ready for human review at the current slice maturity level; the coordinators review notes now exist and the coordinators packet is now ready for human review at the current slice maturity level; the sentence-final particles grammar print slice now exists while dictionary and review-note slices have not yet begun, and broad TAM, directionals, chrestomathy, Mizo/lus, and other Kuki-Chin language work remain deferred.
...
1. [ ] Keep the sentence-final particles grammar slice narrow and move next to `output/publication_review/dictionary_sentence_final_particles_print_slice.md` only if sentence-final particles remains the chosen next packet, without reopening the now review-ready coordinators packet unless a specific reviewer-identified defect appears.
...
3. [ ] Keep broad TAM, directionals, chrestomathy, Mizo/lus, and the other Kuki-Chin languages deferred while the sentence-final particles grammar slice remains the active narrow next step.
