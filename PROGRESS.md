...
- VP structure / suffix stacking now has a narrow grammar print slice at `output/publication_review/grammar_vp_structure_stacking_print_slice.md`, controlled by `output/publication_review/candidates_vp_structure_stacking.tsv` and `output/publication_review/dossier_vp_structure_stacking_scope.md`; the packet stays small and boundary-controlled, using `bawlzoding` as the first print-usable-with-caveat aspect-plus-irrealis stack while keeping `bawlzo` and `pokhia` as already-owned baseline rows and leaving `khia-ta`, `khiathei ding om lo`, `dingin`, `ciahsakkik`, `bawlsakthei`, and `paikhiatsak` explicit as overlap or deferred material rather than broad VP prose.
