...
- Quantifiers now has `output/publication_review/candidates_quantifiers.tsv`, curated extractor support in `scripts/publication_review/extract_candidates.py`, a first interpretive dossier at `output/publication_review/dossier_quantifiers.md`, and a first grammar print slice at `output/publication_review/grammar_quantifiers_print_slice.md`; the packet remains controlled by the candidate/dossier layer, keeps explicit overlap controls for `khat`, `kuamah`, and bang-family `bangmah`, and dictionary/review-note slices have not yet begun.
...
1. [ ] Continue the quantifiers retrofit from `grammar_quantifiers_print_slice.md` into `dictionary_quantifiers_print_slice.md` without broadening into review-note work, coordinators, sentence-final particles, or broad degree/intensifier prose.
