...
- Nominalization now has its first candidate-first scoping packet at `output/publication_review/candidates_nominalization.tsv` plus `output/publication_review/dossier_nominalization_scope.md`. The current packet keeps `-na` as the clearest future nominalization anchor, keeps `-pa` and `-mi` explicit as agentive or person-head material, keeps `omna` visible as the main nominalized-relative overlap row, and keeps `muhna-ah` plus lexicalized `-pa` material such as `kumpipa` explicit as case-routing or lexicalized boundary material rather than forcing an early nominalization grammar slice.
