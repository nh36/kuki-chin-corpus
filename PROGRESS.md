...
- Derivation / valency now has its first candidate-first scoping packet at `output/publication_review/candidates_derivation_valency.tsv` plus `output/publication_review/dossier_derivation_valency_scope.md`; the packet stays small and boundary-controlled, using `paisak`, `muhsak`, `paipih`, and cautiously `kisep` as the clearest future anchors while keeping `mipihte`, `kigen`, `ciahsakkik`, `bawlsakthei`, `paikhiatsak`, and `piangsak` explicit as lexicalized, overlap-heavy, or transitivity-adjacent material rather than early print prose.
- Derivation / valency now also has a first narrow grammar print slice at `output/publication_review/grammar_derivation_valency_print_slice.md`; the slice stays tightly limited to `-sak`, with `paisak` as the causative anchor, `muhsak` as the benefactive or applicative-like split row, the Form I plus `-sak` versus Form II plus `-sak` contrast protected by `tests/test_sak_caus_benf.py`, and `paipih`, `kisep`, `kigen`, `ciahsakkik`, `bawlsakthei`, `paikhiatsak`, `piangsak`, and `mipihte` kept as boundary or deferred material rather than widened into a full derivation or valency chapter.
