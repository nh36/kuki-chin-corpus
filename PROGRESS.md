...
- A third print-review packet on verb stem alternation has now been drafted in `output/publication_review/` with a grammar slice, a dictionary slice, and review notes.
- The stem-alternation packet treats case-marking and pronouns as completed editorial models, argues that the Form I / Form II contrast is real, and keeps auto-selected stem examples under manual review.
- Chrestomathy work and all Mizo/lus work remain deferred while the Tedim print-facing packet is being hardened.
...
1. [ ] Harden the new stem-alternation packet, especially the boundary between safe manual examples and noisy autogenerated report material.
2. [ ] Keep the unresolved `ei/eite` question flagged in later person-marking work unless new evidence settles it.
3. [ ] Decide which narrow print-facing section should follow stem alternation, but do not jump straight to a broad TAM chapter yet.
4. [ ] Only after grammar and dictionary print slices are working, return to the Tedim chrestomathy/reader.
5. [ ] Keep Mizo/lus and the remaining Kuki-Chin languages **deferred, not abandoned**, until the Tedim package is publishable.
