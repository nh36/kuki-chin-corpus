...
- Sentence-final particles now has `output/publication_review/candidates_sentence_final_particles.tsv` plus curated extractor support in `scripts/publication_review/extract_candidates.py`; the first candidate retrofit has begun with narrow `hi`, `lo hi`, `hiam`, `hen`, `in`, `un`, `aw`, `ta`, `tahen`, and `zo` control rows, while dossier and print-slice work have not yet begun.
...
- The quantifiers packet is now ready for human review at the current slice maturity level; the coordinators review notes now exist and the coordinators packet is now ready for human review at the current slice maturity level; the sentence-final particle candidate retrofit has now begun, and broad TAM, directionals, chrestomathy, Mizo/lus, and other Kuki-Chin language work remain deferred.
...
1. [ ] Keep the new sentence-final particle candidate layer narrow and move next to a dossier only if sentence-final particles remains the chosen next packet, without reopening the now review-ready coordinators packet unless a specific reviewer-identified defect appears.
...
3. [ ] Keep broad TAM, directionals, chrestomathy, Mizo/lus, and the other Kuki-Chin languages deferred while the sentence-final particle candidate retrofit remains the active narrow next step.
