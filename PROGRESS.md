...
- Relators / postpositions now has its first narrow grammar print slice as well as its candidate-first scoping layer: `output/publication_review/candidates_relators_postpositions.tsv`, `output/publication_review/dossier_relators_postpositions_scope.md`, and `output/publication_review/grammar_relators_postpositions_print_slice.md` now keep the packet relator-led and case-boundary-controlled around `kiang`, `lak`, `sung`, `tung`, cautiously `pualam`, and boundary-controlled `pan`, `panin`, and `tawh`, while leaving `nuai`, `mai`, `tawhin`, raw report counts, and `kipan` / `kipanin` out of the first core slice.
...
3. [ ] Keep relators / postpositions limited to the candidate TSV plus scoping dossier plus the narrow grammar slice until a later commit explicitly drafts the relators/postpositions dictionary slice.
